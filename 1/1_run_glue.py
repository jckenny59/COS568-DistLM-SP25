...
def train(args, train_dataset, model, tokenizer):
    ...
    model.zero_grad()
    # Change loop variable from "_" to "epoch_itr" for clearer logging.
    train_iterator = trange(int(args.num_train_epochs), desc="Epoch", disable=args.local_rank not in [-1, 0])
    set_seed(args)  # Added here for reproductibility (even between python 2 and 3)
    for epoch_itr in train_iterator:
        ...
        ##################################################
        # Call evaluate() here to get the model performance after every epoch.
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    if args.local_rank not in [-1, 0]:
        torch.distributed.barrier()  # Only the first process processes the dataset; others use the cache
    ...
    if os.path.exists(cached_features_file):
        ...
        if task in ['mnli', 'mnli-mm'] and args.model_type in ['roberta']:
            # Swap label indices for RoBERTa.
...
def main():
    ...
    ##################################################
    # Load the model using from_pretrained (passing in config)
