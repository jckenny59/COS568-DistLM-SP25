...
"""Finetuning the library models for sequence classification on GLUE (Bert, XLM, XLNet, RoBERTa)
   using DistributedDataParallel (DDP). In this version, we run for 1 epoch, discard the timing of the
   first iteration, and report the average iteration time.
...
def train(args, train_dataset, model, tokenizer):
    ...
    # Verify total batch size = per-device batch size * number of workers.
    ...
    optimizer_grouped_parameters = [
        ...
        {'params': [p for n, p in model.module.named_parameters() if any(nd in n for nd in no_decay)],
         'weight_decay': 0.0}
    ]
    ...
    if args.fp16:
        try:
            ...
            raise ImportError("Please install apex from https://github.com/nvidia/apex to use fp16 training.")
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file):
        ...
        if task in ['mnli', 'mnli-mm'] and args.model_type in ['roberta']:
            # For RoBERTa, swap label indices.
...
def main():
    ...
    parser.add_argument("--data_dir", default=None, type=str, required=True,
                        help="Input data directory containing the .tsv files for the task.")
    ...
    parser.add_argument("--cache_dir", default="", type=str,
                        help="Directory to store downloaded pre-trained models")
    ...
    parser.add_argument('--gradient_accumulation_steps', type=int, default=1,
                        help="Steps to accumulate before performing a backward/update pass.")
    ...
    parser.add_argument("--max_steps", default=-1, type=int,
                        help="If > 0, total number of training steps (overrides num_train_epochs).")
