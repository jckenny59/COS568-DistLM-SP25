...
def sync_gradients(model, args): #REMOVED BECAUSE GRADIENT SYNC IS HAPPENING THROUGH PYTORCH DISTRIBUTED
    """Synchronize gradients across all workers via gather and scatter."""
    for param in model.parameters():
        if param.grad is None:
            continue
        grad_tensor = param.grad.data
        # Prepare containers for gathering gradients on rank 0
        if args.local_rank == 0:
            grad_list = [torch.empty_like(grad_tensor) for _ in range(args.world_size)]
        else:
            grad_list = None

        # Gather gradients from all workers to rank 0
        torch.distributed.gather(grad_tensor, grad_list, dst=0)

        # Rank 0 computes the average gradient
        if args.local_rank == 0:
            avg_grad = sum(grad_list) / args.world_size
        else:
            avg_grad = torch.empty_like(grad_tensor)

        # Scatter the averaged gradient from rank 0 back to all workers
        # For scatter, only the source (rank 0) supplies a list; other ranks pass None.
        torch.distributed.scatter(avg_grad, [avg_grad] if args.local_rank == 0 else None, src=0)

        # Update the parameter's gradient with the averaged gradient
        param.grad.data.copy_(avg_grad)

def sync_gradients_all_reduce(model, args): #REMOVED BECAUSE GRADIENT SYNC IS HAPPENING THROUGH PYTORCH DISTRIBUTED
...
def main():
    ...
    #TODO: Joe ADDED DISTRIBUTED: Add new code to handle distributed training. #################################################################### 
    from torch.nn.parallel import DistributedDataParallel as DDP

    if args.world_size > 1 or args.local_rank != -1:
        # For GPU training, specify device_ids and output_device.
